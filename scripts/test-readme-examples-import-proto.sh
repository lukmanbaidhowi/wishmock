...
# One-shot tester for README grpcurl examples using explicit proto imports.
# - Starts the mock server (Bun if available, else Node)
# - Waits for /readiness
# - Executes grpcurl calls with -import-path/-proto (no reflection needed)
...
LOG_FILE="/tmp/mock-grpc.test.import-proto.out"
rm -f "$LOG_FILE"

echo "Starting mock server with RUNNER=$RUNNER (HTTP_PORT=$HTTP_PORT, GRPC_PORT=$GRPC_PORT)" | tee -a "$LOG_FILE"

# Ensure third-party protos (google/type, etc.) are present for imports
